...
public class Verifications extends CommonOps {
    ...
    @Step("verify element not displayed")
    public static void CheckSalaries(List<WebElement> table) {
        int last_min = 0;

        for (int i = 0 ; i < table.size() ; i++) {
            table.get(i).click();
            softAssert.assertFalse(OrangeEditPayGrade.lbl_currency_Name.getText().isEmpty(),OrangeEditPayGrade.txt_payGrade_name.getAttribute("value") + " is not valid");
            softAssert.assertFalse(OrangeEditPayGrade.lbl_min_sallery.getText().isEmpty(),OrangeEditPayGrade.txt_payGrade_name.getAttribute("value") + " is not valid");
            if (i > 0) {softAssert.assertTrue(Integer.parseInt(OrangeEditPayGrade.lbl_max_sallery.getText().split("\\.")[0].replace(",",""))== last_min,OrangeEditPayGrade.txt_payGrade_name.getAttribute("value") + " is not valid");
            }
            last_min = Integer.parseInt(OrangeEditPayGrade.lbl_min_sallery.getText().split("\\.")[0].replace(",",""));
            softAssert.assertFalse(OrangeEditPayGrade.lbl_max_sallery.getText().isEmpty(),OrangeEditPayGrade.txt_payGrade_name.getAttribute("value") + " is not valid");
            softAssert.assertTrue(Integer.parseInt(OrangeEditPayGrade.lbl_max_sallery.getText().split("\\.")[0].replace(",","")) >= Integer.parseInt(OrangeEditPayGrade.lbl_min_sallery.getText().split("\\.")[0].replace(",","")),OrangeEditPayGrade.txt_payGrade_name.getAttribute("value") + " is not valid");
            UIActions.Click(OrangeEditPayGrade.btn_cancel);
        }
